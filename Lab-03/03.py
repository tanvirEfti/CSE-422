...
def minimax_alpha_beta(depth, max_rating, min_rating, alpha, beta, player):
    if depth == 0:
    ...
    if player: 
        max_val = float('-inf')
        for i in range(2): 
            recurr = minimax_alpha_beta(depth - 1, max_rating, min_rating, alpha, beta, False)
        ...
        min_val = float('inf')
        for i in range(2):  
            recurr = minimax_alpha_beta(depth - 1, max_rating, min_rating, alpha, beta, True)
...
branching = 5
max_player_rating = None
min_player_rating = None
start = None
magnus_win = 0
caruana_win = 0
draw = 0

starting_player = int(input(f"Enter starting player for game 1 (0 for Carlsen, 1 for Caruana): "))
base_carlsen = float(input("Enter base rating for Carlsen: "))
base_caruana = float(input("Enter base rating for Caruana: "))

for i in range(4):
    if start is None:
        if starting_player == 0:
            max_player_rating = base_carlsen
            min_player_rating = base_caruana
            start = starting_player
        else:
            max_player_rating = base_caruana
            min_player_rating = base_carlsen
            start = starting_player
    else:
        if start == 0:
            max_player_rating = base_caruana
    ...
    x = minimax_alpha_beta(branching, max_player_rating, min_player_rating, float('-inf'), float('inf'), True)
    
    if start == 0:
        if x > 0:
            magnus_win += 1
            print(f"Game {i + 1} Winner: Magnus Carlsen (Max) (Utility Value: {round(x, 2)})")
        elif x < 0:
            caruana_win += 1
            print(f"Game {i + 1} Winner: Fabiano Caruana (Min) (Utility Value: {round(x, 2)})")
        else:
            draw += 1
    elif start == 1:
        if x > 0:
            caruana_win += 1
            print(f"Game {i + 1} Winner: Fabiano Caruana (Max) (Utility Value: {round(x, 2)})")
        elif x < 0:
            magnus_win += 1  
            print(f"Game {i + 1} Winner: Magnus Carlsen (Min) (Utility Value: {round(x, 2)})") 
        else:   
            draw += 1

print("\n")    
print("Overall results:")
print(f"Magnus Carlsen wins: {magnus_win}")
print(f"Fabiano Caruana wins: {caruana_win}")
print(f"Draws: {draw}")

if magnus_win > caruana_win:
    print("Overall Winner: Magnus Carlsen")
elif caruana_win > magnus_win:
